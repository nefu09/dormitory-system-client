<template>
  <div>
    <div style="width: 100%; height: 50px">
      <el-button type="text" style="margin-top: 20px" @click="toMyHelpReward()"
        >查看我的帮忙帖</el-button
      >
    </div>
    <div style="position: absolute; right: 100px; top: 100px" @click="toAdd()">
      <svg
...
export default defineComponent({
  ...
  setup() {
    ...
    var student: Student = {};
    var myNeedRewards: Reward[] = [];
    var myNeedRewards01 = ref(myNeedRewards);
    ...
    axios.post(`/getSelfNeedRewards/${student.studentNumber}`).then((resp) => {
      if (resp && resp.data.code == 200) {
        if (resp.data.data.rewards != null) {
          myNeedRewards01.value = resp.data.data.rewards;
        }
      }
    });
    ...
    function deleteReward(reward: Reward) {
      axios.post(`/deleteReward/${reward.id}`).then((resp) => {
        if (resp && resp.data.code == 200) {
          axios
            .post(`/getSelfNeedRewards/${student.studentNumber}`)
            .then((resp) => {
              if (resp && resp.data.code == 200) {
                if (resp.data.data.rewards != null) {
                  myNeedRewards01.value = resp.data.data.rewards;
    ...
    function toMyHelpReward() {
      router.replace(`/myHelpReward`);
    }
